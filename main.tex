...
Il doppio livello è essenziale per lo spazio. Salvare un conteggio globale per ogni bucket richiederebbe $\Theta(\log n)$ bit per ogni contatore; dentro un singolo superblocco, invece, un conteggio non supera $\ell^2$, quindi bastano:
\[
    O(\log \ell^2)=O(\log\log n)
\]
bit. \uline{I superblocchi forniscono dunque pochi contatori grandi, mentre i bucket forniscono molti contatori piccoli}.
...
\begin{itemize}
    ...
    \item i primi due caratteri di $BL_6$, cioè \texttt{ca}, nei quali la tabella universale restituisce una sola \texttt{a}.
\end{itemize}
Pertanto:
\[
...
Assumendo che l'alfabeto $\Sigma$ abbia dimensione costante e ricordando che $\ell=\Theta(\log n)$:
\begin{itemize}
    \item esistono $O(n/\ell^2)$ superblocchi; i contatori $NO$ e gli offset $W$ richiedono $O(\log n)$ bit ciascuno, per un totale di $O(n/\log n)$ bit;
    \item esistono $O(n/\ell)$ bucket; i valori relativi $NO'$ e $W'$ sono limitati dalla dimensione di un superblocco e richiedono $O(\log\log n)$ bit ciascuno, per un totale di:
          \[
              O\!\left(\frac{n\log\log n}{\log n}\right)
              \text{ bit};
          \]
    \item uno stato MTF occupa $O(|\Sigma|\log|\Sigma|)=O(1)$ bit, quindi tutti gli stati $MTF_i$ richiedono $O(n/\log n)$ bit.
\end{itemize}

Resta da controllare la dimensione della tabella universale $S$. Un bucket compresso ha lunghezza massima:
\[
    \ell\bigl(1+2\lfloor\log|\Sigma|\rfloor\bigr)
\]
bit. Si sceglie la costante nascosta in $\ell=\Theta(\log n)$ in modo che questa quantità sia:
\[
    \alpha\log n,
    \qquad
    0<\alpha<1.
\]
I possibili blocchi compressi sono allora al più:
\[
    2^{\alpha\log n}=n^\alpha=o(n).
\]
Il numero di possibili stati MTF dipende soltanto da $|\Sigma|$ e viene quindi assorbito nella costante. Poiché per ogni combinazione di blocco e stato MTF si memorizzano $O(\ell)$ contatori di $O(\log\ell)$ bit, la tabella occupa:
\[
    O\!\left(n^\alpha\log n\log\log n\right)
    =
    o\!\left(\frac{n\log\log n}{\log n}\right)
\]
bit. La tabella non vanifica quindi la compressione dell'indice.

In conclusione, tutte le operazioni della query sono accessi diretti a tabelle e parole di macchina. Nel modello RAM con parola di $\Theta(\log n)$ bit e alfabeto costante si ottiene:
\[
    \operatorname{Occ}(c,q)\text{ in tempo }O(1)
\]
con spazio complessivo:
...
Il termine \eqref{eq:fm_occ_space} va infine combinato con lo spazio:
\[
    O\!\left(\frac{n}{\log^\varepsilon n}\right)
\]
dei campioni usati per la localizzazione. Per ogni costante $0<\varepsilon<1$ vale:
\[
