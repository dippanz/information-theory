...
Inoltre, per poter decodificare il messaggio, il ricevitore deve conoscere lo stesso codice usato dall'encoder. Per questo motivo, insieme al corpo compresso, bisogna memorizzare o trasmettere anche la tabella dei codici, oppure una rappresentazione equivalente dell'albero di Huffman. Questo preambolo aumenta la dimensione complessiva del file compresso e può diventare rilevante quando il testo è corto o quando l'alfabeto è grande. Infine, se la tabella o l'albero vengono persi o corrotti, la decompressione può diventare impossibile, perché il flusso di bit non contiene da solo il significato dei simboli originari.
...
La ridondanza dei codici di Huffman è stata studiata in modo approfondito. Senza entrare nei bound tecnici, l'idea principale è che Huffman è molto efficiente, ma resta vincolato al fatto di assegnare una parola di codice intera a ogni singolo simbolo. Per questo, quando la sorgente è estremamente sbilanciata, ad esempio quando un simbolo ha probabilità molto vicina a $1$, l'entropia può essere molto piccola, mentre Huffman deve comunque usare almeno un bit per codificare quel simbolo. In questi casi la lunghezza media può rimanere relativamente lontana dal limite teorico.

\subsection{Esercizio guidato}
** scrivere questo esercizio guidato che riassume tutte le codifiche **
