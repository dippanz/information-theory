...
La formula LF, nella convenzione a indici da $1$, diventa:
\[
    \operatorname{LF}(i)
    =
...
Per rendere $\operatorname{Occ}$ veloce, $L$ viene partizionata logicamente in \textbf{\texten{bucket}} di lunghezza $\ell=\Theta(\log n)$ e in \textbf{\texten{superbucket}} di lunghezza $\ell^2=\Theta(\log^2 n)$. Si memorizzano:
\begin{itemize}
    \item conteggi cumulativi dei caratteri fino alla fine di ogni \texten{superbucket};
    \item conteggi relativi all'interno del \texten{superbucket}, fino alla fine di ogni \texten{bucket};
    \item informazioni per trovare in $Z$ il \texten{bucket} compresso corrispondente;
    \item lo stato della lista MTF all'inizio di ogni \texten{bucket};
    \item una piccola tabella universale per decodificare e contare dentro un singolo \texten{bucket}.
\end{itemize}

Così il calcolo di $\operatorname{Occ}(c,q)$ si divide in tre contributi:
\[
    \operatorname{Occ}(c,q)
    =
    \text{conteggio nei superbucket}
    +
    \text{conteggio nei bucket}
    +
    \text{conteggio dentro il bucket corrente}.
\]
Il primo e il secondo contributo sono letture di tabelle; il terzo usa il \texten{bucket} compresso, lo stato MTF salvato e la tabella universale. Con una scelta opportuna dei parametri, $\operatorname{Occ}$ si calcola in tempo $O(1)$ usando spazio sublineare aggiuntivo oltre al testo compresso.
...
\begin{theorem}[Ferragina-Manzini]
    ...
    \[
        5nH_k(T)+o(n)
    \]
    bit, dove il termine $o(n)$ raccoglie le strutture ausiliarie per $\operatorname{Occ}$ e per il campionamento delle righe.
\end{theorem}
...
Inoltre, la versione pratica di LZ77 con \textbf{finestra limitata} perde parte della forza teorica. Se la finestra ha dimensione fissata $L$, esistono stringhe in cui le ripetizioni utili cadono sempre appena fuori dalla finestra: il compressore allora produce molte frasi corte anche se l'entropia empirica di ordine opportuno è bassa. Per questo LZ77 con finestra limitata non è grossolanamente ottimale in generale.
