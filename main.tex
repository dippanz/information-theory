...
\subsubsection{Urna e moneta: rarità e informazione}
...
\begin{itemize}
    ...
    \item Dati due simboli indipendenti $a_i$ e $a_j$, abbiamo:
          ...
          \end{itemize}
\end{itemize}
\subsubsection{Auto-informazione di una sorgente binaria}
...
\begin{proof}
    ...
    \end{enumerate}
\end{proof}
\subsubsection{Calcolo dell'entropia su sorgenti discrete}
...
\section{Codici di sorgente: decodificabilità e limiti}
...
\end{itemize}
\subsubsection{Verifica di un codice istantaneo}
...
\subsubsection{Codici completi e somma di Kraft}
...
\subsubsection{Verifica della disuguaglianza su codici assegnati}
...
\subsubsection{Costruzione di un codice ternario a prefisso}
...
\subsubsection{Confronto tra codice compatto e non compatto}
...
\end{itemize}
\newpage
\subsubsection{Costruzione di un codice compatto ternario}
...
\section{Algoritmi di codifica statistica}
...
\subsubsection{Sorgenti per cui il codice di Shannon non è compatto}
...
% TODO: aggiungere un esercizio guidato che riassuma le codifiche viste fin qui.
...
\subsubsection{Costo di rappresentazione del codice canonico}
...
\section{Codifica universale degli interi}
...
\subsection{Perché il binario semplice non basta}
...
\subsection{Lunghezze ideali e distribuzioni compatibili}
...
\subsection{Codici di Elias: gamma e delta}
...
\subsection{Codice di Fibonacci}
...
\subsection{Confronto tra i codici}
...
\section{Burrows--Wheeler Transform e compressione}
...
\section{Costruzione della BWT e suffix array}
...
\subsection{Suffix tree e suffix array}
Dato un testo:
\[
...
\subsection{BWT da suffix array}
...
\section{Indicizzazione compressa con FM-index}
...
\subsection{Ricerca tramite suffix array}
...
\subsection{Backward search sulla BWT}
...
\subsection{Da L a Z: la compressione nell'FM-index}
...
\subsection{Come calcolare Occ senza decomprimere}
...
\subsection{Testi ripetitivi e r-index}
...
\subsection{Confronto tra LZ77, LZSS, LZ78 e LZW}
Le quattro varianti discusse fin qui possono essere riassunte così:
\begin{itemize}
...
\section{Informazione algoritmica}
...
\section{Compressione grammaticale}
...
Le regole create dal Re-Pair standard possono contenere terminali, per esempio
$A\to\texttt{ab}$ o $B\to\texttt{a}A$, e il lato destro finale dell'assioma
può avere più di due simboli. Queste produzioni non sono ancora tutte nella
forma:
\[
    A\to BC
    ...
    A\to a
\]
richiesta dalla forma normale di Chomsky.

La \textbf{variante CNF di Re-Pair} evita le regole miste sostituendo, prima di
iniziare, ogni terminale $a$ con un non terminale dedicato $X_a$ e aggiungendo:
\[
    X_a\to a.
\]
Il lato destro ha lunghezza uno, ma contiene un terminale: questa è una
produzione ammessa dalla forma normale di Chomsky e non va confusa con una
produzione unitaria $A\to B$, che avrebbe invece un solo non terminale a
destra.

Re-Pair lavora quindi su una sequenza formata soltanto da non terminali. Ogni
sostituzione successiva crea una regola binaria $A\to XY$, già nella forma
corretta. Al termine rimane soltanto da binarizzare la sequenza residua.

\begin{algorithm}[H]
    \caption{Variante di Re-Pair in forma normale di Chomsky}
    \begin{algorithmic}[1]
        \Require stringa non vuota $w$
        \ForAll{$a\in\operatorname{alph}(w)$}
            \State crea il non terminale $X_a$ e la produzione $X_a\to a$
        \EndFor
        \State sostituisci ogni terminale $a$ di $w$ con $X_a$ e chiama $u$ la sequenza ottenuta
        \While{esiste un digramma con almeno due occorrenze non sovrapposte}
            \State scegli un digramma $XY$ di frequenza massima
            \State crea un nuovo non terminale $A$ con produzione $A\to XY$
            \State sostituisci con $A$ tutte le occorrenze non sovrapposte di $XY$ in $u$
        \EndWhile
        \If{$u=X_1$}
            \State usa $X_1$ come assioma
        \ElsIf{$u=X_1X_2$}
            \State crea l'assioma con produzione $S\to X_1X_2$
        \Else
            \State scrivi $u=X_1X_2\cdots X_h$
            \State crea $S\to X_1Y_1$
            \For{$i=1$ \textbf{to} $h-3$}
                \State crea $Y_i\to X_{i+1}Y_{i+1}$
            \EndFor
            \State crea $Y_{h-2}\to X_{h-1}X_h$
        \EndIf
        \State \Return l'assioma e tutte le produzioni create
    \end{algorithmic}
    \label{alg:repair_cnf}
\end{algorithm}

Per esempio, se la sequenza residua è $X_1X_2X_3X_4$, la fase finale produce:
\[
    S\to X_1Y_1,
    \qquad
    Y_1\to X_2Y_2,
    ...
    Y_2\to X_3X_4.
\]
Se la sequenza residua contiene $h\geq 2$ simboli, questa binarizzazione
richiede complessivamente $h-1$ produzioni, delle quali $h-2$ introducono i
nuovi non terminali $Y_i$. Non modifica la stringa generata e non introduce
produzioni unitarie del tipo $A\to B$.

Come nel Re-Pair standard, quando più digrammi hanno la stessa frequenza
massima la grammatica ottenuta può dipendere dalla regola con cui
l'implementazione risolve i pareggi.

\subsubsection{Confronto sullo stesso esempio}
Riprendiamo:
\[
    w_0=\texttt{aaabcaabaaabcabdabd}
\]
e manteniamo le stesse scelte di digrammi dell'esempio Re-Pair standard. La
variante CNF introduce innanzitutto:
\[
    X_a\to\texttt{a},
    \qquad
    X_b\to\texttt{b},
    ...
    X_d\to\texttt{d}.
\]
La sequenza iniziale diventa quindi una sequenza di soli non terminali. Le
sostituzioni di Re-Pair corrispondono esattamente a quelle già eseguite, con i
terminali rimpiazzati dai rispettivi simboli:
\[
    \begin{array}{c|c|l}
        \textbf{Passo}
        & \textbf{Regola standard}
        & \textbf{Regola nella variante CNF} \\ \hline
        1 & A\to\texttt{ab}   & A\to X_aX_b \\
        2 & B\to\texttt{a}A   & B\to X_aA   \\
        3 & C\to A\texttt{d}  & C\to AX_d   \\
        4 & D\to B\texttt{c}  & D\to BX_c   \\
        5 & E\to\texttt{a}D   & E\to X_aD
    \end{array}
\]
In entrambi i casi la sequenza residua è:
\[
    EBECC.
\]
Nel Re-Pair standard può diventare direttamente il lato destro
$S\to EBECC$. Nella variante CNF, invece, deve essere binarizzata:
\[
    S\to EY_1,
    \qquad
    Y_1\to BY_2,
    ...
    Y_3\to CC.
\]
La grammatica CNF completa è dunque formata da:
\begin{itemize}
    \item le quattro regole terminali $X_a\to\texttt{a}$,
          $X_b\to\texttt{b}$, $X_c\to\texttt{c}$ e
          $X_d\to\texttt{d}$;
    \item le cinque regole binarie $A,B,C,D,E$ della tabella;
    \item le quattro regole binarie $S,Y_1,Y_2,Y_3$ che concatenano la
          sequenza residua.
\end{itemize}
La dimensione della grammatica CNF è:
\[
    |G_{\mathrm{CNF}}|
    =
    ...
    22.
\]
La grammatica standard aveva invece dimensione $15$. L'aumento non indica una
compressione peggiore dell'algoritmo di sostituzione: è il costo strutturale
necessario per rappresentare separatamente i terminali e imporre che ogni
produzione non terminale sia binaria.
...
\begin{definizione}
    Un insieme di posizioni (leggasi gamma):
    \[
...
minimo: in questo caso $\{5,8\}$ è già uno string attractor.
Poiché l'alfabeto è binario, il limite
\eqref{eq:attractor_alphabet_lower_bound} mostra che:
\[
    \gamma^*(f_6)=2.
\]
L'attrattore ottenuto dal parsing contiene dunque quattro posizioni
ridondanti.
...
Il terminatore aumenta la lunghezza del testo di una unità ed è parte della
definizione della BWT usata in questo risultato. Nei risultati seguenti le
formule che coinvolgono $r$ vanno quindi lette sul testo terminato $T^\$$,
anche quando il simbolo è omesso per alleggerire la notazione.
...
L'idea è scegliere al più una posizione rappresentativa per ogni produzione:
il carattere prodotto da una regola terminale, oppure un punto di giunzione tra
le parti di una concatenazione o tra due copie consecutive di una ripetizione.
Una regola di estrazione non richiede invece una nuova posizione: ogni fattore
contenuto nell'estratto può essere riportato alla corrispondente occorrenza
nella stringa sorgente. Ripetendo questo trasferimento si raggiunge
un'\textbf{occorrenza primaria} che attraversa uno dei punti selezionati.
...
$X\to Y^\ell$. Indicando con $g^*_{\mathrm{rl}}(T)$ la dimensione minima in
questo modello, vale il limite più forte:
\begin{equation}\label{eq:run_length_grammar_from_delta}
    g^*_{\mathrm{rl}}(T)
    \in
    O\left(
    \delta(T)
    ...
    \right).
\end{equation}
È questo il risultato con un solo fattore logaritmico. Non sostituisce
\eqref{eq:gstar_from_gamma} per gli SLP ordinari: il modello run-length è più
espressivo, perché una potenza viene descritta da una sola regola. Per gli SLP
ordinari, combinando la conversione da LZ77 con
\eqref{eq:z_from_delta}, si ottiene invece anche:
\[
    g^*(T)
    \in
    O\left(
    \delta(T)
    \log^2\frac{n}{\delta(T)}
    \right).
\]
...
possibile ottenere, nel limite
\eqref{eq:run_length_grammar_from_delta}, grammatiche run-length e strutture
dello stesso ordine di spazio che supportano accesso casuale e, con tecniche
più elaborate, ricerca indicizzata.
...
fattori che attraversano una concatenazione, ottenendo il primo limite.

L'uguaglianza con $w^2$ richiede un argomento più sottile: non è vero che ogni
fattore di $w^q$ compaia già in $w^2$, perché i fattori possono essere più
lunghi di $2|w|$. Si usa invece la periodicità di $w^q$ per traslare, modulo
$|w|$, le posizioni di un attrattore entro copie consecutive di $w$. In questo
modo un attrattore di $w^q$ induce un attrattore non più grande per $w^2$ e,
viceversa, un attrattore di $w^2$ può essere collocato in $w^q$ senza aumentare
la cardinalità. Ne segue l'uguaglianza delle due dimensioni minime.
...
\subsection{Caso di studio: parole di Thue--Morse}
...
\begin{equation}\label{eq:thue_morse_attractor_size}
    i\geq 4
    \quad\Longrightarrow\quad
    \gamma^*(t_i)=4.
\end{equation}
Una costruzione esplicita di un attrattore minimo è:
\[
    \Gamma_i
    =
    \left\{
    ...
    \right\}.
\]
Per $i=4$ produce $\{4,6,8,12\}$, mentre per $i=5$ produce
$\{8,12,16,24\}$. La parola continua a crescere e non è periodica, ma quattro
posizioni ben scelte sono sufficienti a intercettare un'occorrenza di ogni
fattore distinto. Gli altri insiemi di cardinalità quattro riportati nella
tabella mostrano che l'attrattore minimo non è necessariamente unico.

\subsection{Esercizi sugli string attractors}
\subsubsection{Verifica di attrattori minimi}
