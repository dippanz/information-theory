...
\]
dove $0 \leq \theta(C) \leq 1$. Più il valore tende a 1 e più il codice è efficiente, mentre più si discosta da 1 e meno è efficiente. Se $C$ è un codice ottimale allora $\theta(C)=1$.
...
\end{itemize}
\paragraph{Nota:}
Raggiungere l'entropia per un codice $C$ implica che i \textbf{simboli più frequenti} sono codificati con \textbf{parole più corte}, mentre i \textbf{simboli meno frequenti} sono codificati con \textbf{parole più lunghe}. Questa è la base teorica per la costruzione di codici ottimali come il codice di Huffman.
...
\paragraph{Caso di uguaglianza:}
Per un codice univocamente decodificabile (UD) d-ario raggiungere l'entropia implica che $\sum_{i=1}^m d^{-l_i} = 1$, questa uguaglianza viene rispettata \textbf{se e solo se}:
\[
    \left\{
    \begin{aligned}
        p_i & = d^{-l_i},                           \\
    ...
    \qquad i=1,\ldots,m.
\]
Questo è possibile \uline{solo se} le lunghezze delle parole di codice sono numeri interi\footnote{\`E ovvio che le lunghezze delle parole siano numeri interi, queste infatti misurano quanto ogni parola di codifica debba essere lunga. Quindi se questi valori sono numeri interi allora deve valere che anche il logaritmo sia un numero intero.}, ovvero se $l_i=\log_d\!\left(\frac{1}{p_i}\right)\in \mathbb{N}$ per ogni $i$.
