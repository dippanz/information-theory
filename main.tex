...
\begin{itemize}
    ...
    \item le codewords sono ordinate da quella a lunghezza maggiore a quelle a lunghezza minore.
\end{itemize}
...
Nel ciclo for, l'indice $\ell$ rappresenta la lunghezza corrente delle parole di codice. La divisione per $2$ corrisponde a uno shift verso destra: si elimina l'ultimo bit sinistro e si risale quindi dal livello $\ell+1$ al livello $\ell$ dell'albero.
...
\]
Il termine $\text{fc}[\ell+1]+\text{num}[\ell+1]$ rappresenta il primo codice non ancora usato tra quelli lunghi $\ell+1$ bit. Dividere per $2$ equivale a eliminare l'ultimo bit, cioè a risalire di un livello nell'albero. In altre parole, si prende il padre del primo nodo libero al livello successivo.
...
Il risparmio ottenuto attraverso l'uso del codice canonico è significativo, a maggior ragione quando l'alfabeto è grande. Invece di dover memorizzare o trasmettere un albero con puntatori, è sufficiente memorizzare le lunghezze delle parole di codice e costruire il codice canonico in modo ordinato. Infatti, per trasmettere la codifica canonica, è sufficiente inviare l'array $\text{fc}$ e $\text{symb}$. Questo significa che:
\begin{itemize}
    \item per l'array $\text{fc}$, al massimo dovremmo trasmettere $L_{\max}^2$ bits, dove $L_{\max}$ è la lunghezza massima che una codewords può avere (si raggiunge il massimo quando ognuna delle codewords ha lunghezza massima);
    \item per l'array $\text{symb}$, al massimo dovremmo trasmettere $(|A| + L_{\max})\cdot \log_2(|A|+1)$ bits, dove $|A|$ è la dimensione dell'alfabeto (si raggiunge il massimo quando tutte le codewords hanno lunghezza massima, cioè quando $L_{\max}=|A|-1$).
\end{itemize}
