...
\]
Qualunque codice a prefisso binario sui singoli simboli usa però almeno un bit per ogni simbolo. La \textbf{codifica aritmetica} supera questo limite cambiando punto di vista: non associa una parola di codice a ogni simbolo, ma rappresenta l'intero messaggio come un numero reale in un sotto intervallo di $[0,1)$.

L'idea è questa: si parte dall'intervallo $[0,1)$ e, per ogni simbolo letto, lo si suddivide in sotto intervalli proporzionali alle probabilità dei simboli. Il sotto intervallo corrispondente al simbolo corrente diventa il nuovo intervallo corrente. Alla fine, qualunque numero interno all'intervallo finale identifica l'intero messaggio, purché encoder e decoder condividano lo stesso modello probabilistico e lo stesso ordine dei simboli.
...
La lunghezza $n$ è necessaria perché un singolo numero reale non dice da solo quando fermare la decodifica. In alternativa si può aggiungere al modello un simbolo speciale di fine file, spesso indicato come \texttt{EOF}, e fermarsi quando il decoder lo ricostruisce.
...
\[
    ...
    C:[0.9,1).
\]
Codifichiamo il messaggio \texttt{BBBC}. A ogni passo scegliamo il sotto intervallo associato al simbolo corrente:
\begin{center}
    \begin{tabular}{|c|c|c|}
        \hline
        \textbf{Passo} & \textbf{Simbolo letto} & \textbf{Intervallo corrente} \\ \hline
        $0$            & --                     & $[0,1)$                      \\ \hline
        $1$            & $B$                    & $[0.4,0.9)$                  \\ \hline
        $2$            & $B$                    & $[0.6,0.85)$                 \\ \hline
        $3$            & $B$                    & $[0.7,0.825)$                \\ \hline
        $4$            & $C$                    & $[0.8125,0.825)$             \\ \hline
    \end{tabular}
\end{center}
...
\[
    ...
    0.8125.
\]
Seguendo la convenzione delle slide, si può quindi trasmettere la stringa di bit:
\[
    \texttt{1101000}.
\]
Il decoder, conoscendo le probabilità e la lunghezza del messaggio, ripete la stessa suddivisione: il numero $0.8125$ cade prima nel sottointervallo di $B$, poi ancora in quello di $B$, poi ancora in quello di $B$, e infine in quello di $C$. L'output ricostruito è quindi \texttt{BBBC}.

\subsubsection{Algoritmo di decodifica}
La decodifica è simmetrica rispetto alla codifica. L'encoder usa il simbolo corrente per scegliere il sottointervallo; il decoder usa il numero trasmesso per capire in quale sottointervallo cade. Se $x=0.b$ è il numero rappresentato dalla stringa binaria compressa, il decoder parte da $[0,1)$ e ripete la suddivisione per $n$ passi.
...
\begin{center}
    \begin{tabular}{|c|c|c|c|}
        \hline
        \textbf{Passo} & \textbf{Simbolo} & \textbf{Lunghezza $s_i$} & \textbf{Intervallo $[l_i,l_i+s_i)$} \\ \hline
        $0$            & --               & $1$                      & $[0,1)$                             \\ \hline
        $1$            & $a$              & $\frac{1}{2}$            & $\left[0,\frac{1}{2}\right)$        \\ \hline
        $2$            & $b$              & $\frac{1}{8}$            & $\left[\frac{1}{4},\frac{3}{8}\right)$ \\ \hline
        $3$            & $a$              & $\frac{1}{16}$           & $\left[\frac{1}{4},\frac{5}{16}\right)$ \\ \hline
        $4$            & $c$              & $\frac{1}{64}$           & $\left[\frac{19}{64},\frac{5}{16}\right)$ \\ \hline
    \end{tabular}
\end{center}
...
Vediamo anche la decodifica. Il numero $x=\frac{39}{128}$ cade:
\begin{itemize}
    \item in $[0,\frac{1}{2})$, quindi il primo simbolo è $a$;
    \item poi, restringendo l'intervallo a $[0,\frac{1}{2})$, cade in $[\frac{1}{4},\frac{3}{8})$, quindi il secondo simbolo è $b$;
    \item poi, restringendo l'intervallo a $[\frac{1}{4},\frac{3}{8})$, cade in $[\frac{1}{4},\frac{5}{16})$, quindi il terzo simbolo è $a$;
    \item infine cade in $[\frac{19}{64},\frac{5}{16})$, quindi il quarto simbolo è $c$.
\end{itemize}
Il messaggio ricostruito è dunque \texttt{abac}.

\subsubsection{Quanti bit servono?}
Dato un intervallo finale $[l,l+s)$, vogliamo scegliere una frazione dyadica interna usando meno bit possibile. Il criterio usato nelle slide è scegliere il punto centrale $l+s/2$ e troncarne la rappresentazione binaria a un numero sufficiente di bit.
...
\begin{proof}
    ...
    \[
        ...
        2^{-d}.
    \]
    Inoltre la troncatura non può essere maggiore di $x$, perché sostituisce alcuni bit successivi con zeri.
\end{proof}

Applicando il teorema a $x=l+s/2$, basta prendere:
\[
    d=\left\lceil\log_2\frac{2}{s}\right\rceil.
\]
Infatti $2^{-d}\leq s/2$, quindi:
\[
...
Usando \eqref{eq:arithmetic_final_size}, il numero di bit emessi per una sequenza $S$ di lunghezza $n$ è al più:
\[
...
\]
Se le probabilità sono stimate dalle frequenze empiriche della sequenza, raggruppando i termini uguali si ottiene:
\[
    2-\sum_{\sigma\in A} n_\sigma \log_2 p(\sigma)
    =
    2+nH(S),
\]
dove $n_\sigma$ è il numero di occorrenze di $\sigma$ e $H(S)$ è l'entropia della sorgente. In media, quindi, il costo è:
\[
    H(S)+\frac{2}{n}
    \quad \text{bit/simbolo}.
\]
Lo spreco di due bit è sull'intera sequenza, non su ogni simbolo; per messaggi lunghi il termine $2/n$ diventa trascurabile. Questo è il punto in cui la codifica aritmetica supera il limite pratico di Huffman sui singoli simboli.
...
\]
All'inizio le probabilità sono tutte $1/3$. Dopo ogni simbolo, il conteggio del simbolo osservato viene incrementato:
\begin{center}
    \begin{tabular}{|c|c|c|c|}
        \hline
        \textbf{Passo} & \textbf{Simbolo} & \textbf{Probabilità usate $(a,b,c)$} & \textbf{Intervallo scelto} \\ \hline
        $1$            & $b$              & $\left(\frac{1}{3},\frac{1}{3},\frac{1}{3}\right)$ & $\left[\frac{1}{3},\frac{2}{3}\right)$ \\ \hline
        $2$            & $c$              & $\left(\frac{1}{4},\frac{2}{4},\frac{1}{4}\right)$ & $\left[\frac{7}{12},\frac{2}{3}\right)$ \\ \hline
        $3$            & $c$              & $\left(\frac{1}{5},\frac{2}{5},\frac{2}{5}\right)$ & $\left[\frac{19}{30},\frac{2}{3}\right)$ \\ \hline
        $4$            & $b$              & $\left(\frac{1}{6},\frac{2}{6},\frac{3}{6}\right)$ & $\left[\frac{23}{36},\frac{13}{20}\right)$ \\ \hline
    \end{tabular}
\end{center}
