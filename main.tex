...
La difficoltà vera è implementare $\operatorname{Occ}(c,q)$ senza conservare tutta la matrice e, nella versione compressa, senza conservare nemmeno esplicitamente tutta la colonna $L$. Dopo la compressione appena descritta, infatti, l'indice dispone principalmente di $Z$, non di $L$ già pronta da scandire. Tuttavia sia la \texten{backward search} sia il \texten{backward step} richiedono continuamente valori della forma $\operatorname{Occ}(c,q)$.

Il problema diventa quindi: \textbf{come contare le occorrenze di un carattere nei primi $q$ simboli di $L$ guardando solo piccole porzioni della rappresentazione compressa $Z$?}
