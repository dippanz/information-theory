...
\begin{itemize}
    ...
    \item \textbf{autodelimitanti}, cioè decodificabili senza separatori esterni;
...
\paragraph{Codici ottimali rispetto a una distribuzione.}
Un codice è ottimale, rispetto a una distribuzione $p$, quando le lunghezze soddisfano approssimativamente:
\[
    l(x)\simeq \log_2\frac{1}{p(x)}.
\]
Se $l(x)=x$, allora:
\[
    x=\log_2\frac{1}{p(x)}
    ...
    p(x)=2^{-x}.
\]
Dunque il codice unario è adatto quando la probabilità decresce molto rapidamente: i primi interi sono estremamente probabili e gli interi grandi diventano subito trascurabili. Con lo stesso ragionamento, la codifica fissa di \eqref{eq:fixed_length_integer_code} è naturale quando gli interi sono circa uniformi nell'intervallo $\{1,2,\ldots,m\}$.

\subsubsection{Codici di Elias: gamma e delta}
I due codici universali più classici presentati nelle slide sono i codici di Elias, introdotti da Peter Elias nel 1975: il \textbf{codice gamma} e il \textbf{codice delta}. Entrambi partono da una stessa idea: la rappresentazione binaria di $x$ è compatta, ma non dice dove finisce; allora si codifica anche la sua lunghezza in modo autodelimitante.
...
\]
Il codice delta è quindi ancora universale, ma per interi grandi è più corto del gamma.
...
\begin{theorem}[I codici gamma e delta sono a prefisso]
    I codici $\gamma$ e $\delta$ di Elias sono codici a prefisso, quindi sono istantaneamente decodificabili.
\end{theorem}

\begin{proof}
    Nel codice gamma, la sequenza iniziale di zeri determina in modo univoco quanti bit devono essere letti per completare $B(x)$. Dopo aver letto quei bit, la parola è terminata e nessun bit successivo può farne ancora parte. Dunque una parola gamma completa non può essere prefisso di un'altra parola gamma.

    Nel codice delta, la prima parte è una parola gamma che determina in modo univoco il valore $N+1$, cioè la lunghezza della rappresentazione binaria di $x$. Una volta noto $N$, il decoder sa esattamente quanti bit successivi leggere. Anche qui la fine della parola è determinata internamente dalla parola stessa, quindi il codice è a prefisso.
\end{proof}
...
\begin{equation}\label{eq:zeckendorf_representation}
    ...
    \qquad d_i\in\{0,1\},
\end{equation}
e la cifra più a destra usata è $d_{k-1}=1$, il codice di Fibonacci è:
\begin{equation}\label{eq:fibonacci_code}
    \operatorname{Fib}(N)=d_0d_1\cdots d_{k-1}1.
\end{equation}
L'ultimo $1$ è un bit aggiuntivo di terminazione.

\paragraph{Perché è a prefisso.}
Nella rappresentazione di Zeckendorf non compaiono mai due $1$ consecutivi. Aggiungendo un $1$ finale, ogni parola di codice termina invece con la sottostringa \texttt{11}. Poiché \texttt{11} non può comparire all'interno della rappresentazione, il decoder riconosce la fine della parola appena vede due $1$ consecutivi. Per questo il codice è a prefisso.
...
Dalla decomposizione di Zeckendorf:
\[
    73=55+13+5
    =
    F_{10}+F_7+F_5.
...
\]
Aggiungendo il bit finale di terminazione:
\[
...
\]
è il rapporto aureo. Inoltre è un codice \textbf{sincronizzante}: la sottostringa \texttt{11} permette di riconoscere il confine della parola di codice. Questo è utile in presenza di errori, perché dopo una corruzione locale il decoder può ritrovare un punto di allineamento leggendo fino al successivo terminatore.
