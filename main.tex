...
\begin{proof}
    ...
    \[
        \sum_{i=1}^m p_i \log\!\left(\frac{1}{p_i}\right) - \sum_{i=1}^m p_i \log\!\left(\frac{1}{q_i}\right) =
    ...
    \[
        \log\!\left(\sum_{j=1}^m d^{-l_j}\right) + L_S(C)\cdot \log d \leq L_S(C)\cdot \log d
    \]
...
\subsubsection{Proprietà}
\begin{itemize}
    \item \textbf{Criterio di ottimalità:} Se per un certo codice $C$ \uline{la lunghezza media di un codice eguaglia l'entropia} allora si può affermare che quel codice è ottimale, ovvero è il codice più efficiente possibile per rappresentare la sorgente $S$. Essendo l'entropia il limite inferiore per la lunghezza media, raggiungere questo limite significa che non esiste alcun altro codice che possa rappresentare la sorgente con una lunghezza media più bassa. Viceversa, se un codice ha una lunghezza media superiore all'entropia, allora presenta un certo margine di inefficienza.

          Raggiungere l'entropia per un codice $C$ implica che i simboli più frequenti (con probabilità più alta) sono codificati con parole più corte, mentre i simboli meno frequenti (con probabilità più bassa) sono codificati con parole più lunghe. Questa è la base teorica per la costruzione di codici ottimali come il codice di Huffman.

    \item \textbf{Caso di uguaglianza:} Per un codice univocamente decodificabile (UD) d-ario raggiungere l'entropia implica che $\sum_{i=1}^m d^{-l_i} = 1$ e l'uguaglianza viene rispettata \textbf{se e solo se}:
          \[
              p_i=d^{-l_i}
              \quad\text{e}\quad
              l_i=\log_d \frac{1}{p_i}
          \]
          per ogni $i$. Questo è possibile \uline{sole se} le lunghezze delle parole di codice sono numeri interi\footnote{\`E ovvio che le lunghezze delle parole siano numeri interi, queste infatti misurano quanto ogni parola di codifica debba essere lunga. Quindi se questi valori sono numeri interi allora deve valere che anche il logaritmo sia un numero intero.}, ovvero se $l_i=\log_d \frac{1}{p_i}\in \mathbb{N}$ per ogni $i$.

          Si nota che scegliendo:
          \[
              p_i=\left(\frac{1}{d}\right)^{\alpha_i}=d^{-\alpha_i} \quad\text{con}\quad \alpha_i > 0 \in \mathbb{N}
          \]
    ...
    \item \textbf{Efficienza:} Per misurare l'efficienza di un codice $C$ si può usare la seguente formula:
          \[
              \theta(C) = \frac{H_d(S)}{L_S(C)} = \frac{H(S)}{L_S(C)\cdot \log d}
          \]
          dove $0 \leq \theta(C) \leq 1$. Più il valore tende a 1 è più il codice è efficiente, mentre più si discosta da 1 è meno efficiente. Se $C$ è un codice ottimale allora $\theta(C)=1$.

    \item \textbf{Ridondanza:} Per misurare la ridondanza di un codice $C$ si può usare la seguente formula:
          \[
              \rho(C) = 1- \theta(C) = \frac{L_S(C) - H_d(S)}{L_S(C)}
          \]
          dove $0 \leq \rho(C) \leq 1$. Più il valore tende a 1 è più il codice è ridondante, mentre più si discosta da 1 è meno ridondante. Se $C$ è un codice ottimale allora $\rho(C)=0$. La ridondanza rappresenta la porzione di lunghezza media che non è strettamente necessaria per rappresentare l'informazione della sorgente, ma che è invece dovuta a inefficienze nella progettazione del codice. Un codice con alta ridondanza utilizza più simboli di codifica del necessario, mentre un codice con bassa ridondanza si avvicina al limite inferiore stabilito dall'entropia della sorgente. Questa serve a misurare quanto un codice si discosta dall'ottimalità.
\end{itemize}
