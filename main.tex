...
\begin{itemize}
    \item \textbf{\texten{suffix tree}}: struttura compatta simile a un albero compresso; ogni cammino dalla radice a una foglia rappresenta un suffisso;
    \item \textbf{\texten{suffix array}}: array ordinato dei punti di partenza dei suffissi.
\end{itemize}

Entrambe hanno dimensione lineare in $n$, ma il \texten{suffix array} è spesso più conveniente in memoria. 

\paragraph{L'ordine di grandezza pratico:} Un \texten{suffix array} occupa circa $4n$ \texten{byte} se gli interi occupano 4 \texten{byte}, mentre un \texten{suffix tree} può occupare circa $16n$ \texten{byte}. Su testi enormi, come genomi da molti \texten{gigabyte}, questa differenza diventa decisiva.
