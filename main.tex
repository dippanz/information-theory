...
\subsubsection{\texorpdfstring{Ricerca tramite \texten{suffix array}}{Ricerca tramite suffix array}}
...
\subsubsection{\texorpdfstring{\texten{Backward search} sulla BWT}{Backward search sulla BWT}}
...
\]
Essa manda una riga della matrice ordinata alla riga che rappresenta il suffisso precedente nel testo.
...
Nel testo \texttt{mississippi\#}, tutti i suffissi che iniziano con \texttt{i} occupano un intervallo contiguo della matrice ordinata. Per cercare \texttt{si}, la ricerca parte dall'ultimo carattere \texttt{i}, individua l'intervallo dei suffissi che iniziano con \texttt{i}, poi restringe quell'intervallo guardando in $L$ quali righe sono precedute da \texttt{s}. Le formule \eqref{eq:fm_backward_first} e \eqref{eq:fm_backward_last} traducono esattamente questo restringimento.
...
\[
    ...
    [9,10].
\]
Quindi \texttt{si} compare $10-9+1=2$ volte. Al contrario, se durante un'estensione si ottiene $\textit{First}>\textit{Last}$, l'intervallo è vuoto: per esempio, continuando verso un pattern non presente, l'ultimo aggiornamento può produrre un estremo sinistro maggiore dell'estremo destro.

\subsubsection{\texorpdfstring{Da \texten{count} a \texten{locate}}{Da count a locate}}
...
Nella versione compressa non vogliamo memorizzare esplicitamente $L[i]$. Per calcolare comunque il \texten{backward step}, si può identificare il carattere $c=L[i]$ confrontando:
\[
...
\]
Se $c=\#$, la rotazione attraversa il simbolo di fine testo e la posizione corrispondente è la prima posizione del testo, considerando il testo in modo circolare. Quando l'alfabeto è costante e $\operatorname{Occ}$ costa $O(1)$, anche il \texten{backward step} costa $O(1)$.

Per localizzare le occorrenze senza memorizzare tutto il \texten{suffix array}, l'FM-index campiona alcune righe. Per esempio si marcano le righe corrispondenti a posizioni del testo multiple di:
\[
    \left\lceil \log^{1+\varepsilon} n\right\rceil,
    \qquad
    0<\varepsilon<1,
\]
e per queste righe si conserva la posizione nel testo. Per localizzare una riga non marcata, si applica LF finché si raggiunge una riga marcata; se sono serviti $t$ passi e la riga marcata ha posizione $q$, allora la posizione originaria è $q+t$ modulo $n$.

Il numero di passi è al più:
...
\]
e per ciascuna bisogna conservare almeno la posizione nel testo. Aumentare $\varepsilon$ riduce il numero di campioni e quindi lo spazio, ma aumenta il numero massimo di \texten{backward step} necessari per localizzare una posizione.

\subsubsection{\texorpdfstring{Come calcolare \texten{Occ} senza decomprimere}{Come calcolare Occ senza decomprimere}}
La difficoltà vera è implementare $\operatorname{Occ}(c,q)$ senza conservare tutta la matrice e, nella versione compressa, senza conservare nemmeno esplicitamente tutta la colonna $L$. Nella pipeline considerata dall'FM-index, il testo viene trasformato in:
\[
    ...
    \operatorname{PC}(
        \operatorname{RLE}(
            \operatorname{MTF}(
                \operatorname{BWT}(T)))).
\]
Qui $\operatorname{PC}$ indica un codice a prefisso a lunghezza variabile applicato dopo BWT, MTF e RLE. L'RLE usato in questa pipeline codifica le run di zeri prodotte dalla MTF: una run di lunghezza $m$ viene rappresentata tramite la scrittura binaria di $m+1$, letta dal bit meno significativo e privata del bit più significativo. Per esempio:
\[
    0\mapsto 0,
    \qquad
    00\mapsto 1,
    ...
    000\mapsto 00.
\]
La decodifica segue la stessa idea al contrario: se il codice della run è $b_0b_1\cdots b_t$, letto nell'ordine in cui è stato scritto, la lunghezza recuperata è:
\[
    m=\sum_{j=0}^{t}(b_j+1)2^j.
\]
Il codice a prefisso finale usa due codici brevi per i simboli speciali introdotti dall'RLE, per esempio:
\[
...
\[
    0^{\lfloor\log(a+1)\rfloor}\operatorname{bin}(a+1),
\]
...
Per rendere $\operatorname{Occ}$ veloce, $L$ viene partizionata logicamente in \textbf{\texten{bucket}} di lunghezza $\ell=\Theta(\log n)$ e in \textbf{\texten{superbucket}} di lunghezza $\ell^2=\Theta(\log^2 n)$. Per semplificare l'esposizione, si fa in modo che una run di zeri non attraversi il confine tra due \texten{bucket}; altrimenti bisogna conservare anche l'informazione necessaria a proseguire la run nel blocco successivo.

Le strutture ausiliarie principali sono:
\begin{itemize}
    \item $NO_t[c]$: conteggi cumulativi dei caratteri fino alla fine del $t$-esimo \texten{superbucket};
    \item $NO'_i[c]$: conteggi relativi all'interno del \texten{superbucket} corrente, fino alla fine del \texten{bucket} $i$;
    \item $W_t$ e $W'_i$: informazioni sulle lunghezze compresse dei blocchi, usate per trovare in $Z$ il \texten{bucket} compresso $BZ_i$;
    \item $MTF_i$: stato della lista MTF all'inizio del \texten{bucket} $i$;
    \item $S[c,h,BZ_i,MTF_i]$: tabella universale che restituisce quante occorrenze di $c$ compaiono nei primi $h$ caratteri del \texten{bucket} decompresso.
\end{itemize}

Sia $i=\lceil q/\ell\rceil$ il \texten{bucket} che contiene $q$, sia $h=q-(i-1)\ell$ la posizione interna al \texten{bucket}, e sia $t$ il numero di \texten{superbucket} completi prima di $i$. Allora il calcolo di $\operatorname{Occ}(c,q)$ si divide in tre contributi:
\[
    \operatorname{Occ}(c,q)
    =
    NO_t[c]
    ...
    S[c,h,BZ_i,MTF_i].
\]
Il primo e il secondo contributo sono letture di tabelle; il terzo usa il \texten{bucket} compresso, lo stato MTF salvato e la tabella universale. Le strutture $NO$ e $W$ richiedono $O(n/\log n)$ bit, mentre $NO'$ e $W'$ richiedono $O((n/\log n)\log\log n)$ bit. Gli stati MTF costano ancora $O(n/\log n)$ bit. La tabella universale viene mantenuta sublineare scegliendo la dimensione effettiva del \texten{bucket} in modo che il numero di possibili blocchi compressi sia $n^\alpha$, con $\alpha<1$. Con una scelta opportuna dei parametri, $\operatorname{Occ}$ si calcola in tempo $O(1)$ usando:
\[
    Z+O\!\left(\frac{n\log\log n}{\log n}\right)
\]
bit oltre alla rappresentazione compressa principale.
...
\subsubsection{\texorpdfstring{Testi ripetitivi e \texten{r-index}}{Testi ripetitivi e r-index}}
