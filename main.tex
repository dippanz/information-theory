...
\[
    ...
    \right).
\]
Partendo da $I=6$ e leggendo $L[\operatorname{LF}^i[I]]$ si ottengono i caratteri nell'ordine inverso: prima \texttt{s}, poi \texttt{c}, poi \texttt{i}, e così via, fino a ricostruire \texttt{mathematics}.
...
Per valutare un compressore basato su BWT non basta dire che ``produce run''. Serve una misura quantitativa. Le slide introducono l'\textbf{entropia empirica}, che misura la compressibilità del testo a partire dalle frequenze osservate nel testo stesso, senza assumere un modello probabilistico della sorgente.
...
\end{align}
dove $\mu$ è una piccola costante dovuta alle implementazioni a precisione finita della codifica aritmetica.

\subsubsection{Entropia empirica di ordine k}
L'ordine zero non cattura la dipendenza dal contesto. In un testo naturale, la distribuzione del prossimo simbolo cambia se il contesto precedente è \texttt{th}, \texttt{qu}, \texttt{zz}, oppure un altro fattore. Per questo si definisce l'entropia empirica di ordine $k$.

Sia $A$ l'alfabeto e sia $w\in A^k$ un contesto di lunghezza $k$. Indichiamo con $w_s$ la stringa formata dai simboli che seguono le occorrenze di $w$ in $s$. Allora:
\begin{equation}\label{eq:k_order_empirical_entropy}
    ...
    |w_s|H_0(w_s).
\end{equation}
La stessa idea può essere definita considerando i simboli che precedono il contesto, invece di quelli che lo seguono. Il valore $|s|H_k(s)$ è un limite ideale per un compressore che usa memoria di lunghezza $k$.
...
Il risultato teorico importante, dovuto a Manzini, collega la BWT all'entropia empirica di ordine $k$. L'idea intuitiva è che la BWT raggruppa nella colonna $L$ simboli che condividono contesti simili; se si riuscisse a partizionare perfettamente $L$ secondo questi contesti, un compressore di ordine zero su ogni blocco si comporterebbe come un compressore di ordine $k$ sul testo originale.
...
Se fossimo in grado di trovare esattamente questa partizione e avessimo un compressore $A$ tale che:
\[
...
allora:
\[
    |A(\operatorname{BWT}(s))|\leq |s|H_k(s).
\]
In pratica, un compressore basato su BWT cerca di approssimare sia questa partizione sia il comportamento ideale del compressore $A$.

Le slide riportano due limiti utili. Sia $\operatorname{Order}_0$ un compressore di ordine zero tale che, per una costante $\mu$:
\[
...
\begin{equation}\label{eq:manzini_bwt0_bound}
    ...
    h^k(2h\log h+9),
\end{equation}
dove $h$ è la dimensione dell'alfabeto.
...
\begin{equation}\label{eq:manzini_bwtrl_bound}
    ...
    (5+3\mu)|s|H_k^*(s)+g_k,
\end{equation}
dove $g_k$ dipende solo da $k$ e dalla dimensione dell'alfabeto $h$.

Questi risultati non dicono soltanto che la BWT ``funziona bene in pratica'': mostrano che una pipeline BWT-based può essere analizzata rispetto a misure empiriche del testo, quindi senza assumere una sorgente probabilistica prefissata.

\subsection{Calcolo efficiente della BWT}
La definizione tramite rotazioni è chiara ma inefficiente. Per una parola lunga $n$, costruire e ordinare esplicitamente $n$ rotazioni di lunghezza $n$ può richiedere molto più di tempo lineare e molta memoria. Il collo di bottiglia è proprio l'ordinamento delle rotazioni.

La strategia standard consiste nell'aggiungere al testo un simbolo speciale di fine stringa:
\[
    T=s\texttt{\$},
\]
minore di tutti gli altri simboli e presente una sola volta. In questo modo l'ordinamento delle rotazioni cicliche di $T$ può essere ricondotto all'ordinamento dei suffissi di $T$.
...
Due strutture dati classiche per memorizzare o ordinare i suffissi sono:
\begin{itemize}
    \item \textbf{\texten{suffix tree}}: struttura compatta simile a un \texten{trie} compresso; ogni cammino dalla radice a una foglia rappresenta un suffisso;
    \item \textbf{\texten{suffix array}}: array ordinato dei punti di partenza dei suffissi.
\end{itemize}

Entrambe hanno dimensione lineare in $n$, ma il \texten{suffix array} è spesso più conveniente in memoria. Le slide riportano l'ordine di grandezza pratico: un \texten{suffix array} occupa circa $4n$ \texten{byte} se gli interi occupano 4 \texten{byte}, mentre un \texten{suffix tree} può occupare circa $16n$ \texten{byte}. Su testi enormi, come genomi da molti \texten{gigabyte}, questa differenza diventa decisiva.
...
\[
    T=\texttt{banana\$},
\]
il \texten{suffix array} contiene le posizioni iniziali dei suffissi ordinati. Per esempio, il suffisso \texttt{\$} viene prima di tutti, poi vengono i suffissi che iniziano con \texttt{a}, poi quelli che iniziano con \texttt{b} e \texttt{n}. L'array non memorizza i suffissi, ma solo le loro posizioni iniziali.

\subsubsection{\texorpdfstring{BWT da \texten{suffix array}}{BWT da suffix array}}
Una volta costruito il \texten{suffix array} di $T=s\texttt{\$}$, la BWT si ottiene in modo diretto. Se $SA[r]=i$, allora la riga di rango $r$ corrisponde al suffisso che parte da $i$; il carattere della colonna $L$ è il carattere che precede quel suffisso nel testo, cioè:
\begin{equation}\label{eq:bwt_from_suffix_array}
    ...
    \end{cases}
\end{equation}
Il secondo caso gestisce la rotazione ciclica: se il suffisso parte dall'inizio del testo, il carattere precedente è il simbolo finale.
...
\[
    SA=(6,5,2,3,0,4,1).
\]
Applicando \eqref{eq:bwt_from_suffix_array}:
\[
    \operatorname{BWT}(T)=\texttt{abba\$aa}.
\]
Questo è il valore riportato nelle slide. Il vantaggio è che non abbiamo dovuto ordinare esplicitamente tutte le rotazioni: è sufficiente ordinare i suffissi.

\subsection{\texorpdfstring{Algoritmo DC3 per costruire il \texten{suffix array}}{Algoritmo DC3 per costruire il suffix array}}
Il tempo di calcolo della BWT dipende dal tempo necessario a costruire il \texten{suffix array}. I primi algoritmi lineari per la costruzione del \texten{suffix array} sono apparsi nel 2003, circa dieci anni dopo l'introduzione della BWT. Le slide citano due famiglie importanti:
\begin{itemize}
    \item \textbf{DC3}, basato su \texten{difference cover sampling};
    \item \textbf{SA-IS}, basato su \texten{induced sorting}.
\end{itemize}
Qui ci concentriamo su DC3, introdotto da Karkkainen e Sanders e poi sviluppato con Burkhardt.
...
DC3 usa le classi di resto modulo $3$. Le posizioni vengono divise in:
\[
    P_{1,2}=\{i\mid i\bmod 3=1 \text{ oppure } i\bmod 3=2\},
    \qquad
    P_0=\{i\mid i\bmod 3=0\}.
\]
Il campione $P_{1,2}$ contiene circa $\frac{2}{3}n$ posizioni; il resto $P_0$ contiene circa $\frac{1}{3}n$ posizioni.
...
\begin{algorithm}[htpb]
    ...
    \begin{algorithmic}[1]
        ...
        \State $P_{1,2}\gets \{i\mid i\bmod 3\in\{1,2\}\}$
        \State costruisci una stringa ridotta $T^{1,2}$ che codifica i suffissi che partono in $P_{1,2}$
        \State costruisci ricorsivamente il \texten{suffix array} di $T^{1,2}$
        \State ricava $SA^{1,2}$, cioè l'ordine dei suffissi che partono in $P_{1,2}$
        \State costruisci $SA^0$, cioè l'ordine dei suffissi che partono in $P_0$, usando coppie $\langle T[i],\operatorname{pos}(i+1)\rangle$
        \State fondi $SA^{1,2}$ e $SA^0$ con confronti in tempo costante
        \State \Return $SA$
    \end{algorithmic}
\end{algorithm}

\paragraph{Passo 1: ordinare i suffissi in $P_{1,2}$.}
Si costruisce una stringa ridotta $T^{1,2}$ lunga circa $\frac{2}{3}n$. L'idea è raggruppare il testo in triplette partendo dalle posizioni congruenti a $1$ e $2$ modulo $3$, poi sostituire ogni tripletta con il suo rango lessicografico.
...
\[
    ...
    [\texttt{\$ \$ \$}].
\]
Ordinando le triplette con \texten{radix sort} e assegnando a ogni tripletta distinta il suo rango:
\[
    [\texttt{\$ \$ \$}]<[\texttt{i \$ \$}]<[\texttt{i p p}]<[\texttt{i s s}]<[\texttt{p p i}]<[\texttt{s s i}],
\]
si ottiene la stringa di ranghi:
\[
    T^{1,2}=(3,3,2,1,5,5,4,0).
\]
Se tutti i simboli di $T^{1,2}$ fossero distinti, i suffissi di $T^{1,2}$ sarebbero ordinabili guardando solo il primo simbolo. Poiché in questo esempio ci sono ripetizioni, si costruisce ricorsivamente il \texten{suffix array} di $T^{1,2}$. Le slide riportano:
\[
    SA(T^{1,2})=(7,3,2,1,0,6,5,4).
\]
Convertendo le posizioni di $T^{1,2}$ nelle posizioni corrispondenti del testo originale:
\[
    SA^{1,2}=(11,10,7,4,1,8,5,2).
\]

\paragraph{Passo 2: ordinare i suffissi in $P_0$.}
Se $i\in P_0$, allora $i+1\in P_{1,2}$. Quindi il suffisso:
\[
    T[i,n)
\]
può essere rappresentato dalla coppia:
\[
    \langle T[i],\operatorname{pos}(i+1)\rangle,
\]
dove $\operatorname{pos}(i+1)$ è il rango del suffisso $T[i+1,n)$ dentro $SA^{1,2}$. Poniamo anche $\operatorname{pos}(n+1)=0$.
...
\[
    \begin{array}{rcl}
        0 &\mapsto& \langle \texttt{m},4\rangle,\\
        3 &\mapsto& \langle \texttt{s},3\rangle,\\
        6 &\mapsto& \langle \texttt{s},2\rangle,\\
        9 &\mapsto& \langle \texttt{p},1\rangle.
    \end{array}
\]
...
Resta da fondere $SA^{1,2}$ e $SA^0$. Il problema è che un suffisso in $SA^0$ e un suffisso in $SA^{1,2}$ appartengono a due liste diverse, quindi non conosciamo ancora il loro ordine relativo. DC3 evita confronti carattere per carattere usando i ranghi già calcolati.

Sia $i\in P_0$ e sia $j\in P_{1,2}$. Ci sono due casi:
\begin{itemize}
