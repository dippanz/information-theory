...
\begin{figure}[htpb]
    ...
    \caption{Finestra scorrevole di LZ77 con un esempio integrato. Il prefisso \texttt{aba} del \texten{look-ahead buffer} compare già nel \texten{search buffer}; l'encoder può quindi emettere offset $o=i-j=5$, lunghezza $\ell=3$ e simbolo successivo $s=\texttt{a}$.}
    \label{fig:lz77_sliding_window_example}
\end{figure}

Alla posizione corrente l'encoder cerca nel \texten{search buffer} la sottostringa più lunga che coincide con un prefisso del \texten{look-ahead buffer}, come in \autoref{fig:lz77_sliding_window_example}. Se trova un match, lo rappresenta tramite una tripla:
\[
