...
La definizione non cerca il programma più veloce, ma quello con la descrizione
...
Una stringa è algoritmicamente casuale quando non possiede una descrizione
significativamente più corta della propria rappresentazione letterale.

\begin{definizione}
    Una stringa $x$ è \textbf{$c$-incomprimibile} se:
    \[
        K(x)\geq |x|-c.
    \]
    In tal caso si dice anche che $x$ è
    \textbf{algoritmicamente casuale al livello $c$}. La convenzione più
    rigida $K(x)\geq |x|$ corrisponde al caso $c=0$.
\end{definizione}

La costante è necessaria perché il teorema di invarianza permette a due
macchine universali di differire di un costo fisso. La nozione rilevante non è
quindi l'uguaglianza esatta tra $K(x)$ e $|x|$, ma l'impossibilità di risparmiare
un numero crescente di bit. Per una stringa finita la casualità va sempre
intesa rispetto alla tolleranza $c$; la nozione di casualità per sequenze
infinite richiede invece definizioni ulteriori, che non sono necessarie qui.
...
\[
    2^n-2^{n-c}+1
\]
stringhe di lunghezza $n$ sono $c$-incomprimibili. Per una stringa scelta
uniformemente a caso, l'incomprimibilità non è quindi un'eccezione, ma il
