...
Il tempo di calcolo della BWT dipende dal tempo necessario a costruire il \texten{suffix array}. Se il \texten{suffix array} si costruisce in tempo lineare, allora anche la fase BWT, una volta noto l'array, richiede solo tempo lineare. I primi algoritmi lineari per la costruzione del \texten{suffix array} sono apparsi nel 2003, circa dieci anni dopo l'introduzione della BWT.

Le slide collocano DC3 dentro il quadro più generale della \textbf{\texten{suffix array construction}}. In particolare citano:
\begin{itemize}
...
\end{itemize}
Qui ci concentriamo su DC3, introdotto da Karkkainen e Sanders e poi sviluppato con Burkhardt, perché è l'algoritmo trattato più in dettaglio nelle slide.

\subsubsection{Schema ricorsivo generale}
La parte chiamata nelle slide \emph{\texten{recursive suffix array construction}} non riguarda la ricostruzione della BWT, ma la costruzione ricorsiva del \texten{suffix array}. L'idea è ordinare prima una parte dei suffissi, codificare quell'ordine in una stringa più corta e poi usare tale informazione per ordinare tutti i suffissi del testo.
...
\autoref{fig:dc3_overview} riassume il flusso logico. La parte blu è quella ricorsiva: dai suffissi campionati si costruisce una stringa ridotta e se ne calcola il \texten{suffix array}. La parte arancione ordina i suffissi non campionati usando i ranghi già noti. La parte verde fonde le due liste ordinate.
...
\]
Per convertire questo risultato in posizioni del testo originale si usa il modo in cui $T^{1,2}$ è stato costruito. Le prime quattro posizioni corrispondono ai suffissi che partono da $1,4,7,10$, mentre le ultime quattro corrispondono ai suffissi che partono da $2,5,8,11$:
\[
