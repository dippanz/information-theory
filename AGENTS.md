...
When the user asks to consult course slides, first resolve paths relative to the repository root and then to the parent course directory (`..`). Verify the file with `Test-Path -LiteralPath` or `Get-Item -LiteralPath`. For PDF slides, prefer `pdfinfo`, `pdftotext -layout`, and, when tables or diagrams are not extracted clearly, render the relevant pages with `pdftoppm` and inspect the generated images. Preserve those temporary rendered slide images while the current notes paragraph or topic is still being developed, because they may be useful in the immediate future; clean them up only after the paragraph is finished or when the user asks. If older instructions or links mention absolute user-profile paths from another machine, treat them as machine-specific and obsolete; use the repository-relative or parent-directory location instead.
