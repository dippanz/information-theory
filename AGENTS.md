...
This repository is a single-document LaTeX project. [`main.tex`](main.tex) is the entry point and contains the preamble, theorem environments, custom macros, and the full course text. Store figures in [`images/`](images/). Import figures in LaTeX as vector PDFs through `\includegraphics`; if a figure has an editable source (for example a TikZ/standalone `.tex` file or an original `.svg`), keep that source next to the exported PDF but do not reference the SVG directly from `main.tex`.
...
The parent course directory (`..`, resolved from the repository root) contains the teaching material for the subject, organized in numbered folders and related archives. Treat that material as a reference source for the project: consult it when the user explicitly asks to align with the slides, when a theorem/definition/order of topics needs to be checked against the course material, or when verifying notation or examples would materially improve accuracy. Do not depend on those files continuously by default, because the notes are meant to be a re-elaboration of the material rather than a direct transcription.

When the user asks to consult course slides, first resolve paths relative to the repository root and then to the parent course directory (`..`). If the requested slides are not there, also check the course OneDrive folder at `C:\Users\emanu\OneDrive - UNIPA\materie magistrale\inf theory\`, especially numbered subfolders such as `05_information_theory_data_compression`. Verify the file with `Test-Path -LiteralPath` or `Get-Item -LiteralPath`. For PDF slides, prefer `pdfinfo`, `pdftotext -layout`, and, when tables or diagrams are not extracted clearly, render the relevant pages with `pdftoppm` and inspect the generated images. Preserve those temporary rendered slide images while the current notes paragraph or topic is still being developed, because they may be useful in the immediate future; clean them up only after the paragraph is finished or when the user asks. If older instructions or links mention absolute user-profile paths from another machine, treat them as machine-specific and obsolete; use the repository-relative, parent-directory, or current OneDrive course location instead.
...
## Testing Guidelines
There is no automated test suite in this repository. Validation means a clean LaTeX build and a quick visual review of [`main.pdf`](main.pdf). Before submitting changes, run the full build, check for broken references, and confirm that equations, theorem environments, tables, forest diagrams, and imported vector figures render correctly.
